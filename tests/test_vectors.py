...
@responses.activate
def test_query_with_vectors_kwarg_sends_flag_and_parses_response():
    """server-sqve.2: with_vectors=True sends include_vectors and parses Result.vector."""
    responses.add(
        responses.POST,
        f"{HOST}/api/v1/query",
        json={
            "results": [
                {"key": "v1", "score": 0.9, "vector": [0.1, 0.2, 0.3]},
                {"key": "v2", "score": 0.7, "vector": [0.4, 0.5, 0.6]},
            ],
            "worker_stats": [],
        },
        status=200,
    ...
    c = make_client()
    results = c.vectors.query("col", vector=[0.1, 0.2, 0.3], with_vectors=True)
...
@responses.activate
def test_query_default_no_vectors_field():
    """server-sqve.2: default behavior omits include_vectors; Result.vector is None."""
    responses.add(
        responses.POST,
        f"{HOST}/api/v1/query",
    ...
    import json
    sent = json.loads(body)
    assert "include_vectors" not in sent

    assert results[0].vector is None


@responses.activate
def test_query_default_min_score_is_no_filter():
    """server-jan0: default min_score=-1.0 returns everything in score order.

    Cosine scores are raw [-1, 1] (not normalized 0-1, per
    project_query_score_convention.md). The previous 0.0 default silently
    dropped mildly-anti-correlated results from broad queries — bad for
    LangChain-style similarity_search(k=N) semantics where callers expect
    'up to N most-similar results' regardless of relevance floor.
    """
    responses.add(
        responses.POST,
        f"{HOST}/api/v1/query",
        json={"results": [{"key": "v1", "score": -0.05}], "worker_stats": []},
        status=200,
    )
    c = make_client()
    ...
    import json
    sent = json.loads(body)
    assert sent["similarity_threshold"] == -1.0
...
@responses.activate
def test_query_502_raises_server_error():
    # server-sqom.6: 502 surfaces as ServerError (not QueryError) because
    # the 502 path is generic — it can hit on status-polls during upsert
    # too, where 'query service unavailable' would be misleading.
    responses.add(
        responses.POST,
        f"{HOST}/api/v1/query",
...
@responses.activate
def test_query_batch_applies_single_query_defaults():
    """server-5pbm.1.6 / server-jan0: each batch item must carry the same
    defaults vp.vectors.query() applies — similarity_threshold=-1.0 (no filter,
    since cosine scores are raw [-1, 1]), distance_metric='cosine',
    include_metadata=True — so the batch path doesn't silently differ from N
    parallel single queries. Without this the coord defaults
    (similarity_threshold=0.7, include_metadata=False) drop hits and strip
    metadata from the batch response."""
    import json
    responses.add(
        responses.POST,
        f"{HOST}/api/v1/query/batch",
        json={"results": [{"status": 200, "results": []}]},
        status=200,
    )
    c = make_client()
    c.vectors.query_batch([{"collection": "col", "vector": [0.1, 0.2]}])
    body = json.loads(responses.calls[0].request.body)
    sent = body["queries"][0]
    assert sent["similarity_threshold"] == -1.0
    assert sent["distance_metric"] == "cosine"
    assert sent["include_metadata"] is True
    # User-supplied fields still win when present.
    c.vectors.query_batch([{"collection": "col", "vector": [0.1], "similarity_threshold": 0.5}])
    body2 = json.loads(responses.calls[1].request.body)
    assert body2["queries"][0]["similarity_threshold"] == 0.5
...
# server-gzb.3.2: extension whitelist + size sanity check.

def test_upsert_rejects_unsupported_extension(tmp_path):
    c = make_client()
    f = tmp_path / "data.txt"
    f.write_bytes(b"not a vector file")
    with pytest.raises(ValidationError, match="Unsupported file extension"):
        c.vectors.upsert("col", f)


def test_upsert_rejects_empty_file(tmp_path):
    c = make_client()
    f = tmp_path / "data.parquet"
    f.write_bytes(b"")
...
@pytest.mark.parametrize("ext", ["csv", "jsonl", "fvecs", "fbin", "npy", "hdf5", "safetensors", "arrow"])
@responses.activate
def test_upsert_accepts_non_parquet_formats(tmp_path, ext):
    """server-gzb.3.2: SDK upload() accepts every format the source-side
    detector handles, not just .parquet. Server does the real validation."""
    c = make_client()
    f = tmp_path / f"data.{ext}"
    payload = b"some-format-bytes-the-sdk-doesnt-parse"
    f.write_bytes(payload)

    responses.add(
        responses.POST,
        f"{HOST}/api/v1/collections/c/files/data.{ext}/uploads",
        json={"upload_id": "up_x", "chunk_size_max": 64 * 1024 * 1024},
        status=201,
    )
    responses.add(
        responses.PUT,
        f"{HOST}/api/v1/uploads/up_x/parts/1",
        json={},
        status=200,
    )
    responses.add(
        responses.POST,
        f"{HOST}/api/v1/uploads/up_x/complete",
        json={"collection": "c", "filename": f"data.{ext}", "size": len(payload)},
        status=201,
    )
    responses.add(
        responses.GET,
        f"{HOST}/api/v1/collections/c/files",
        json={"files": []},
        status=200,
    )
    responses.add(
    ...
    result = c.vectors.upsert("c", f)
    assert result.filename == f"data.{ext}"
...
def test_replace(tmp_path):
    # server-5pbm.1.2.3: replace now uses the chunked upload protocol with
    # method='PUT', identical to upsert except for the verb stamped on the
    # session at start.
    f = tmp_path / "data.parquet"
    ...
    responses.add(
        responses.POST,
        f"{HOST}/api/v1/collections/col/files/data.parquet/uploads",
        json={"upload_id": "up_rep", "chunk_size_max": 67108864},
        status=201,
    )
    responses.add(
        responses.PUT,
        f"{HOST}/api/v1/uploads/up_rep/parts/1",
        json={"ok": True},
        status=200,
    )
    responses.add(
        responses.POST,
        f"{HOST}/api/v1/uploads/up_rep/complete",
        json={"collection": "col", "filename": "data.parquet", "size": 15},
        status=201,
    )
    ...
    # Confirm the start request marked this session as a PUT-mode replace.
    start_call = next(
        call for call in responses.calls
        if call.request.url.endswith("/files/data.parquet/uploads")
    )
    assert b'"method": "PUT"' in start_call.request.body or '"method":"PUT"' in start_call.request.body.decode("utf-8", errors="replace")


@responses.activate
def test_replace_not_found(tmp_path):
    # When the underlying file doesn't exist on source, source's replace_file
    # returns 404 — propagated by consumer-site's chunked-complete handler.
    f = tmp_path / "data.parquet"
    f.write_bytes(b"data")
    ...
    responses.add(
        responses.POST,
        f"{HOST}/api/v1/collections/col/files/data.parquet/uploads",
        json={"upload_id": "up_rnf", "chunk_size_max": 67108864},
        status=201,
    )
    responses.add(
        responses.PUT,
        f"{HOST}/api/v1/uploads/up_rnf/parts/1",
        json={"ok": True},
        status=200,
    )
    responses.add(
        responses.POST,
        f"{HOST}/api/v1/uploads/up_rnf/complete",
        json={"error": "File not found"},
        status=404,
    )
    # The SDK will best-effort DELETE the upload session on the raised
    # exception path; mock it so the cleanup attempt doesn't crash the test.
    responses.add(
        responses.DELETE,
        f"{HOST}/api/v1/uploads/up_rnf",
        status=204,
    )
...
@responses.activate
def test_list_files_includes_wal_synthetic_entry():
    """server-5pbm.1.5.3: when consumer-site reports a __wal__ row with
    is_wal=true and vector_count, the SDK surfaces both fields on FileInfo."""
    responses.add(
        responses.GET,
        f"{HOST}/api/v1/collections/col/files",
        json={
    ...
    artifact = files[0]
    wal = files[1]
    assert artifact.is_wal is False
    assert artifact.vector_count is None
    assert wal.is_wal is True
    assert wal.vector_count == 3
    assert wal.name == "__wal__"

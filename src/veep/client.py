...
# server-sqom.1: substrings that mark an api_key as an obvious placeholder.
# Matched case-insensitively against the value the customer passed. The
# patterns target the literal placeholder in the README's old quickstart and
# the most common ad-hoc variants we've seen in support tickets.
...
def _looks_like_placeholder_key(api_key: str) -> bool:
    upper = api_key.upper()
    return any(marker in upper for marker in _PLACEHOLDER_API_KEY_MARKERS)
...
class VP:
    ...
    def __init__(
        ...
        # server-sqom.1: catch obvious placeholder values BEFORE we hit the
        # network. The README's old quickstart leads with veep_live_REPLACE_ME,
        # and copy-paste customers were getting a generic AuthError on their
        # very first call. Surface the actual mistake instead.
        if _looks_like_placeholder_key(self.api_key):
            raise AuthError(
                f"The api_key '{self.api_key}' looks like a placeholder, not a real key.\n"
                "Run vp = VP.login() for interactive sign-in (works in terminals,\n"
                "Jupyter, and SSH), or paste your real key from https://vectorpanda.com\n"
                "(during private beta you'll need an invite first)."
            )
    ...
    def ping(self) -> bool:
        """Liveness check: is the Vector Panda service reachable?

        This is a TCP-ping equivalent over HTTP. It hits the public
        ``/api/v1/health`` endpoint, which is unauthenticated by
        design — status pages and monitoring probes use it. A bad
        API key will NOT cause ping to fail. Use :meth:`whoami` to
        validate that an API key is recognized.
    ...
    def whoami(self) -> dict:
        """Authenticated identity check: returns the resolved client.

        Hits ``GET /api/v1/whoami``, which authenticates via the
        ``Authorization: Bearer`` header. Use this to verify an API
        key is valid (and which client it resolves to) without the
        side effects of calling :meth:`collections.list` or
        :meth:`vectors.query`.

        Returns:
            A dict with ``client_id`` and ``client_name``.
    ...
    def login(
        ...
        Args:
            ...
            timeout_s: How long to wait for the user to complete the device
                flow. Default 600s — first-time OAuth with 2FA can exceed
                the previous 300s cap (server-sqom.3).
            on_device_code: Optional callback fired with (device_code,
                verification_url) after the device endpoint returns and before
                polling begins. When set, the SDK suppresses its own stdout
                output of the URL — the callback is the canonical surface for
                that information. Useful for programmatic flows that want to
                deliver the URL via a different transport (Slack, email, custom
                UI, automated test approval).
    ...
    def _request(
        ...
        *,
        ...
        if resp.status_code == 409:
            ...
            # server-or56: consumer-site relays coord's structured 409 body with
            # a `code` field — `recently_deleted` for the post-delete cooldown,
            # `already_exists` for a name in use. Distinguishing them lets
            # callers handle "wait and retry" separately from "this name is
            # gone for good." Fall back to AlreadyExists for legacy 409s.
            body: dict = {}
            try:
                body = resp.json() or {}
            except Exception:
            ...
            collection = (
                body.get("collection")
                or _extract_collection_from_path(path)
                or (json["collection"] if json and "collection" in json else None)
            )
            code = body.get("code")
            if code == "recently_deleted":
                raise CollectionRecentlyDeletedError(
                    collection or "unknown",
                    retry_after_secs=body.get("retry_after_secs"),
                )
            if collection:
        ...
        if resp.status_code == 502:
            # server-sqom.6: a 502 can hit on any endpoint (status check,
            # list, query, etc.), so the old QueryError-with-"query service"
            # framing was misleading when 502 surfaced during, e.g., an
            # upsert's status-poll. Use ServerError(502) instead — generic,
            # honest, and lets callers contextualize.
            raise ServerError(
                "Vector Panda is temporarily unavailable (HTTP 502). "
                "This usually clears in a few seconds — try again, or check "
                "service status if it persists.",
                status_code=502,
            )
